...
export default function ProfilePage() {
  // ✅ FIX: was "updateUserProfile" — correct name is "updateUserProfileData"
  ...
  const [isPublic,    setIsPublic]    = useState(true); // ✅ FIX: track isPublic
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      // ✅ FIX: read isPublic from profile (default true if not set)
  ...
  const handleSave = async () => {
    setSaving(true);
    // ✅ FIX: call correct function name, include isPublic
  ...
  const handleCancel = () => {
    // Reset fields back to saved profile on cancel
    if (userProfile) {
      setDisplayName(userProfile.displayName || user?.displayName || "");
      setBio(userProfile.bio || "");
      setLocation(userProfile.location || "");
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={S.heroCard}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
              ...
              {/* ✅ FIX: show public/private badge so user knows their status */}
              <span style={{
                ...S.heroTag,
        ...
        >
          {[
            ...
            <div key={label} style={S.statCard}>
              ...
              <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <div style={S.twoCol}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} style={S.card}>
                  ...
                  {GOALS.map((g) => (
                    <button
                      ...
                      style={{
                        ...
                        borderColor: goal === g ? "rgba(29,229,200,0.3)" : "var(--border)",
            ...
            {!editing && goal && (
              <div style={{ marginBottom: 18 }}>
                <label style={S.fieldLabel}>Primary goal</label>
                <div style={S.fieldValue}>{goal}</div>
              </div>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }} style={S.card}>
            ...
            <div style={S.accountRow}>
              <span style={S.accountLabel}>Account type</span>
              <span style={S.accountValue}>{user?.providerData?.[0]?.providerId === "google.com" ? "Google" : user?.providerData?.[0]?.providerId === "github.com" ? "GitHub" : "Email"}</span>
            </div>
            <div style={S.accountRow}>
            ...
            {/* ✅ FIX: Public profile toggle — this is what makes users appear in search */}
            <div style={{ ...S.accountRow, borderBottom: "none", marginTop: 4 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>Public profile</div>
                <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 2 }}>
                  Appear in search results and community leaderboard
                </div>
              </div>
              <button
                onClick={async () => {
                  const next = !isPublic;
                  setIsPublic(next);
                  // Save immediately without entering edit mode
                  await updateUserProfileData({ isPublic: next });
                }}
                style={{
                  ...S.toggleBtn,
                  background: isPublic ? "var(--teal)" : "var(--surface)",
                  borderColor: isPublic ? "var(--teal)" : "var(--border2)",
                }}
              >
                <div style={{
                  width: 14, height: 14, borderRadius: "50%", background: "#fff",
                  transform: isPublic ? "translateX(16px)" : "translateX(2px)",
...
const S = {
  ...
  heroCard:       { background: "var(--bg2)", border: "0.5px solid var(--border)", borderRadius: 18, padding: "32px", display: "flex", alignItems: "flex-start", gap: 24 },
  avatar:         { width: 72, height: 72, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--border2)" },
  avatarFallback: { width: 72, height: 72, borderRadius: "50%", background: "var(--teal)", color: "#07101e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 800 },
  onlineDot:      { position: "absolute", bottom: 3, right: 3, width: 12, height: 12, borderRadius: "50%", background: "#22c55e", border: "2px solid var(--bg2)" },
  heroName: { fontSize: 22, fontWeight: 800, letterSpacing: "-0.6px", margin: 0, color: "var(--text)" },
  heroSub:  { fontSize: 13, color: "var(--text3)", margin: "4px 0 0" },
  heroBio:  { fontSize: 13, color: "var(--text2)", margin: "10px 0 0", lineHeight: 1.65 },
  ...
  statCard: { background: "var(--bg2)", border: "0.5px solid var(--border)", borderRadius: 14, padding: "20px 18px" },

  twoCol:    { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 },
  card:      { background: "var(--bg2)", border: "0.5px solid var(--border)", borderRadius: 16, padding: "24px" },
  ...
  toggleBtn: { width: 36, height: 20, borderRadius: 10, border: "1.5px solid", cursor: "pointer", flexShrink: 0, position: "relative", display: "flex", alignItems: "center", transition: "background 0.2s, border-color 0.2s", padding: 0 },
  ...
  cancelBtn:  { padding: "9px 20px", background: "transparent", border: "0.5px solid var(--border2)", borderRadius: 10, color: "var(--text2)", fontSize: 13, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 },
