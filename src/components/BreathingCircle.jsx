 ...
 *   onPhaseChange  — (phase: string) => void  — live phase for ExercisePage tracker
 *   onBadgesEarned — (badges: array) => void  — called when new badges are unlocked
 */
const BreathingCircle = ({ index, isRunning, onToggle, onPhaseChange, onBadgesEarned }) => {
  ...
  const persistSession = useCallback(async () => {
    ...
    try {
      ...
      // Process gamification — returns newly unlocked badges
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      >
        <svg width="200" height="200" viewBox="0 0 200 200" style={{ display: "block" }}>
          ...
          />
          <circle cx="100" cy="100" r="54" fill="rgba(29,229,200,0.04)" stroke="rgba(29,229,200,0.10)" strokeWidth="0.5" />
        </svg>
      ...
      <AnimatePresence mode="wait">
        <motion.p
          ...
          style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: meta.color, marginTop: 18, transition: "color 0.4s ease" }}
        >
          {phase}
        </motion.p>
      </AnimatePresence>
